...
class ValidationBuilder implements ValidationInterface
{
	 ...
	 * @param string $rules The rules to use for validation.
	 *
	 * @return mixed
	 */
	public function single($data, $rules)
	{
		$rules = $this->makeTheRules( $rules );
	 ...
	 *
	 * @param array $data
	 * @param array $rules     Divided by `|` Example min:3|required
	 *
	 * @return array
	 */
	 ...
	 * @author rumur
	 */
	protected function makeTheRules($rules)
	{
	...
	/**
	 * Alias for the `textfield` validation.
	 *
	 * @param       $data
	 * @param array $attributes
	 *
	 ...
	 * @author rumur
	 */
	protected function validate_text($data, array $attributes = [])
	{
		return $this->validate_textfield( $data, $attributes );
	}
	 ...
	 * @return string
	 */
	protected function validate_password($data, array $attributes = [])
	{
		$_rules = [
			'num'      => '/[0-9]+/',
			'upper'    => '/[A-Z]+/',
		...
		$rules = empty( $attributes ) ? [ 'notempty' ] : $attributes;

		$result = array_filter( array_map( function ( $rule ) use ( $data, $_rules ) {
			$current_rule = isset( $_rules[ $rule ] ) ? $_rules[ $rule ] : $_rules['notempty'];

			return preg_match( $current_rule, $data );
		}, $rules ) );

		// Check if all rules are passed
		return ( count( $result ) == count( $rules ) ) ? $data : '';
	}

	/**
	 * Validate a text field price value.
	 *
	 * @param string $data       The data to validate.
	 * @param array  $attributes
	 *
	 * @return string
	 */
	protected function validate_price($data, array $attributes = [])
	{
